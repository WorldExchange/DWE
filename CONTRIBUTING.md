...
The DWE Core project operates an open contributor model where anyone is
...
The following applies to code changes to the DWE Core project, and is not to be
confused with overall DWE Network Protocol consensus changes.

Whether a pull request is merged into DWE Core rests with the project merge
...
Patches that change DWE consensus rules are considerably more involved than
...
Where a patch set proposes to change the DWE consensus, it must have been
...
The project leader is the release manager for each DWE Core release.
